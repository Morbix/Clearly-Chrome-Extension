...
{
    //  define JSON parse
      var evalJSON = function (_data) { return JSON.parse(_data); };


    //  include original json-rpc library
    /*
     * evalJSON
     * safely parse JSON data into a JSON object
     * if evalJSON is not defined by the includer of the file,
     * define it here as a standard eval
     */
    if (evalJSON); else
    {
    ...
    */
    ...
    $R.JSONRpcClient = JSONRpcClient;
        
    //  custom error handler
    $R.JSONRpcClient.toplevel_ex_handler = function (_e)
    {
        // unhandled exception thrown in jsonrpc handler
        var a, str="";
        for(a in _e) { str += a+"\t"+_e[a]+"\n"; }
        ...
        try
        {
            switch (true)
            {
                case (!!console && !!console.log): console.log(_e); console.log(str); break;
                case (!!Firebug && !!Firebug.Console && !!Firebug.Console.log): Firebug.Console.log(_e); Firebug.Console.log(str); break;
            }
        }
        catch (e) {}
    };
